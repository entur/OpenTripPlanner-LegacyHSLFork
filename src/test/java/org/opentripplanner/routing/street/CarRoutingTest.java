...
public class CarRoutingTest {
  ...
  public void shouldAllowLoopCausedByTurnRestrictions() {
    ...
    var polyline = computePolyline(hindenburgStrUnderConstruction, transitModel, gueltsteinerStr, aufDemGraben);
  ...
  public void shouldRespectGeneralNoThroughTraffic() {
    ...
    var polyline1 = computePolyline(herrenbergGraph, herrenbergTransitModel, mozartStr, fritzLeharStr);
    assertThatPolylinesAreEqual(polyline1, "_grgHkcfu@OjBC\\ARGjAKzAfBz@j@n@Rk@E}D");

    var polyline2 = computePolyline(herrenbergGraph, herrenbergTransitModel, fritzLeharStr, mozartStr);
  ...
  public void shouldRespectMotorCarNoThru() {
    ...
    var polyline1 = computePolyline(herrenbergGraph, herrenbergTransitModel, schiessmauer, zeppelinStr);
    assertThatPolylinesAreEqual(
      polyline1,
      "otrgH{cbu@v@|D?bAElBEv@Cj@APGAY?YD]Fm@X_@Pw@d@eAn@k@VM@]He@Fo@Bi@??c@?Q@gD?Q?Q@mD?S"
    );

    var polyline2 = computePolyline(herrenbergGraph, herrenbergTransitModel, zeppelinStr, schiessmauer);
    assertThatPolylinesAreEqual(
      polyline2,
  ...
  public void planningFromNoThroughTrafficPlaceTest() {
    ...
    var polyline1 = computePolyline(herrenbergGraph, herrenbergTransitModel, noThroughTrafficPlace, destination);
    assertThatPolylinesAreEqual(
      polyline1,
      "corgHkncu@OEYUOMH?J?LINMNMHTDO@YMm@HS`A}BPGRWLYDEt@HJ@b@?Fc@DONm@t@OXCBz@B\\"
    );

    var polyline2 = computePolyline(herrenbergGraph, herrenbergTransitModel, destination, noThroughTrafficPlace);
    assertThatPolylinesAreEqual(
      polyline2,
  ...
  private static String computePolyline(Graph graph, TransitModel transitModel, GenericLocation from, GenericLocation to) {
